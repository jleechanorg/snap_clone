...
# It automatically navigates to the new worktree after creation.
#
...
if [ -n "$1" ]; then
    ...
    read -p "Enter the desired name for the new worktree (e.g., 'feature-branch'): " WORKTREE_NAME
fi

# Validate that a name was entered
if [ -z "$WORKTREE_NAME" ]; then
    error_exit "Worktree name cannot be empty. Please provide a name."
fi
...
if git show-ref --verify --quiet "refs/heads/$WORKTREE_NAME"; then
    error_exit "A local Git branch named '$WORKTREE_NAME' already exists. Please choose a different name for the worktree to create a new, unique branch."
fi
